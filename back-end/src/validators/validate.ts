    ...
    if (error instanceof ZodError) {
      const details = error.issues.map((err: ZodIssue) => {
        ...
        if (err.code === 'invalid_type') {
          const invalidTypeErr = err as ZodIssue & { received?: string; expected?: string };
          ...
          message = `Le champ '${field}' doit contenir au moins ${(err as any).minimum} caractères.`;
        } else if (err.code === 'too_big') {
          message = `Le champ '${field}' ne peut pas dépasser ${(err as any).maximum} caractères.`;
        }
