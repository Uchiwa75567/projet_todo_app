...
export class TaskController {
  ...
  // --- GET ALL PAGINATED ---
  async getAllTasksPaginated(req: Request, res: Response) {
    try {
  ...
  async updateTask(req: AuthRequest, res: Response) {
    try {
      ...
      if (bodyData.completed !== undefined) updateData.completed = bodyData.completed === 'true' || bodyData.completed === true;

      // Handle image
      if (files?.image?.[0]) {
        if (existing.image) this.deleteFile(existing.image);
      ...
      // Handle file
      if (files?.file?.[0]) {
        if (existing.file) this.deleteFile(existing.file);
      ...
      // Handle voice
      if (files?.voice?.[0]) {
        if (existing.voice) this.deleteFile(existing.voice);
  ...
  async grantPermission(req: AuthRequest, res: Response) {
    try {
      ...
      const permission = await this.taskService.grantPermission(
        taskId,
        ownerId,
        userId,
